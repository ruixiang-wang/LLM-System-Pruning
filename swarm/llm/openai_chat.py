...
if "DEFAULT_HOST" in os.environ and "DEFAULT_PORT" in os.environ:
    OPENAI_URL = f"http://{os.environ['DEFAULT_HOST']}:{os.environ['DEFAULT_PORT']}/v1"
else:
    OPENAI_URL = None

load_dotenv()
OPENAI_API_KEYS = [os.getenv(f"OPENAI_API_KEY", "EMPTY")]
for i in range(10):
...
def openai_chat(
    ...
    if model == "lmstudio":
        ...
        if OPENAI_URL is not None:
            api_kwargs["base_url"] = OPENAI_URL
...
class OpenAIChat(LLM):
    ...
    async def agen(
        ...
        return await openai_achat(
            self.model_name, messages, max_tokens, temperature, num_comps
        )
